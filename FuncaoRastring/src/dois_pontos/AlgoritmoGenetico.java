...
public class AlgoritmoGenetico {
    ...
    public AlgoritmoGenetico(Integer tamanho, Double pCrossover, Double pMutacao, Integer geracoes, Problema problemaDois_pontos, Double minimo, Double maximo, Integer nVariaveis) {
        this.tamanho = tamanho;
        this.pCrossover = pCrossover;
        this.pMutacao = pMutacao;
        this.geracoes = geracoes;
        this.problema = problemaDois_pontos;
        this.minimo = minimo;
        this.maximo = maximo;
        ...
        for (int g = 1; g <= geracoes; g++) {
            ...
            for (int i = 0; i < this.tamanho; i++) {
                ...
                if (rnd.nextDouble() <= this.pCrossover) {
                    ...
                    // Ponto de corte
                    int corte1 = rnd.nextInt(p1.getVariaveis().size());
                    int corte2 = rnd.nextInt(p1.getVariaveis().size());
                    while(corte2<corte1){
                    	corte2 = rnd.nextInt(p1.getVariaveis().size());
                    }
                   
                    // Descendente 1 -> Ind1_1 + Ind2_2;
                    crossoverDoisPontos(p1, p2, desc1, corte1,corte2);

                    // Descendente 2 -> Ind2_1 + Ind1_2;
                    crossoverDoisPontos(p2, p1, desc2, corte1,corte2);
...
//                    + populacao.getIndividuos().get(0).getFuncaoObjetivo());
    ...
    private void crossoverDoisPontos(Individuo ind1, Individuo ind2, Individuo descendente, int corte1, int corte2) {
        ...
        for (int i = 0; i < corte1; i++) {
            Double valor = beta * ind1.getVariaveis().get(i);
            descendente.getVariaveis().add(valor);
        }

        for (int i = corte1; i < corte2; i++) {
            Double valor = (1 - beta) * ind2.getVariaveis().get(i);
            descendente.getVariaveis().add(valor);
        }
        for (int i = corte2; i < this.nVariaveis; i++) {
            Double valor = ind1.getVariaveis().get(i);
            descendente.getVariaveis().add(valor);
        }
    }
